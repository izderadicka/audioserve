...
fn create_parser() -> Command {
    let mut parser = Command::new(crate_name!())
        ...
        .arg(long_arg!(ARG_THREAD_POOL_KEEP_ALIVE_SECS)
            .num_args(1)
        ...
        .arg(long_arg!(ARG_HELP_DIR_OPTIONS)
            .action(ArgAction::SetTrue)
            .help("Prints help for collections directories options")
            )
        .arg(long_arg!(ARG_HELP_TAGS)
            .action(ArgAction::SetTrue)
        ...
        .arg(long_arg!(ARG_NO_AUTHENTICATION)
            .action(ArgAction::SetTrue)
            .help("no authentication required - mainly for testing purposes")
            )
        .arg(long_arg!(ARG_SHARED_SECRET)
            .short('s')
                        .num_args(1)
        ...
        .arg(long_arg!(ARG_SHARED_SECRET_FILE)
            .num_args(1)
            // .conflicts_with(ARG_NO_AUTHENTICATION)
        ...
        .arg(long_arg!(ARG_TRANSCODING_MAX_PARALLEL_PROCESSES)
            .short('x')
                        .num_args(1)
        ...
        .arg(long_arg!(ARG_TRANSCODING_MAX_RUNTIME)
            .num_args(1)
        ...
        .arg(long_arg!(ARG_CHAPTERS_DURATION)
            .num_args(1)
            .value_parser(value_parser!(u32))
            .env("AUDIOSERVE_CHAPTERS_FROM_DURATION")
        ...
        .arg(long_arg!(ARG_URL_PATH_PREFIX)
            .num_args(1)
        ...
        .arg(long_arg!(ARG_STATIC_RESOURCE_CACHE_AGE)
        ...
        .arg(long_arg!(ARG_FOLDER_FILE_CACHE_AGE)
    ...
    // deprecated
    parser = parser.arg(long_arg!(ARG_SEARCH_CACHE).long(ARG_SEARCH_CACHE).help(
    ...
    if cfg!(feature = "tls") {
        parser = parser.arg(long_arg!(ARG_SSL_KEY)
            .num_args(1)
            ...
            .arg(long_arg!(ARG_SSL_CERT)
            .requires(ARG_SSL_KEY)
            .value_parser(is_existing_file)
            .env("AUDIOSERVE_SSL_CERT")
            )
            .arg(long_arg!(ARG_SSL_KEY_PASSWORD)
                .num_args(1)
    ...
    if cfg!(feature = "shared-positions") {
        parser = parser.arg(
            long_arg!(ARG_POSITIONS_BACKUP_FILE)
            .num_args(1)
            .value_parser(parent_dir_exists)
        ...
        .arg(
            long_arg!(ARG_POSITIONS_WS_TIMEOUT)
            .value_parser(duration_secs)
        ...
        .arg(
            long_arg!(ARG_POSITIONS_RESTORE)
            .num_args(1)
        ...
        .arg(
            long_arg!(ARG_POSITIONS_BACKUP_SCHEDULE)
            .num_args(1)
