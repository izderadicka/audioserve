use std::path::Path;
use std::fs;
use super::types::{SearchResult, AudioFolderShort};
...
impl SearchTrait for FoldersSearch {
    fn search<P: AsRef<Path>, S: AsRef<str>>(&self, base_dir: P, query: S) -> SearchResult {
        let tokens: Vec<String> = query.as_ref().split(" ").filter(|s| s.len() > 0).map(|s| s.to_lowercase()).collect();
...
impl FoldersSearch {
    fn search_recursive(base_path: &Path,path: &Path, results: &mut SearchResult, tokens: &Vec<String>) {
                        ...
                        if ft.is_dir() {
                            ...
                            if let Ok(s) = p.clone().into_os_string().into_string() {
                                ...
                                if m {
                                    ...
                                    FoldersSearch::search_recursive(base_path, &p, results, tokens)
                                }
                            }
                        }
                     }
